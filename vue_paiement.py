...
import controleur_paiement
from controleur_paiement import GRILLE_TARIFAIRE
...
class GesProPaiementApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("GesPro - Caisse Scolaire (Scolarité Mensuelle)")
        self.geometry("740x640")
        self.resizable(False, False)

        # Initialisation du support de stockage (géré par le contrôleur)
        controleur_paiement.initialiser_stockage()
        ...
        self.lbl_titre = ctk.CTkLabel(
            self.head_frame, 
            text="PORTAIL DE CAISSE - SCOLARITÉ MENSUELLE (BTS & LICENCE)", 
        ...
        # Ligne 2 : Année d'études & Montant
        ...
        self.lbl_montant = ctk.CTkLabel(self.form_frame, text="Montant de la Mensualité (FCFA) :", font=("Helvetica", 11, "bold"))
        ...
        # --- SECTION AUTRES PAIEMENTS (bibliothèque, examen, etc.) ---
        self.autres_frame = ctk.CTkFrame(self, fg_color="#F8FAFC", border_width=1, border_color="#E2E8F0")
        self.autres_frame.pack(fill="x", padx=45, pady=(0, 15))

        self.lbl_autres = ctk.CTkLabel(
            self.autres_frame,
            text="Autres paiements (bibliothèque, examen, ...) ?",
            font=("Helvetica", 11),
            text_color="#475569"
        )
        self.lbl_autres.pack(side="left", padx=(15, 10), pady=12)

        self.btn_autres_paiements = ctk.CTkButton(
            self.autres_frame,
            text="Accéder à Autres Paiements →",
            font=("Helvetica", 11, "bold"),
            fg_color="#1E3A8A",
            hover_color="#1E40AF",
            height=32,
            command=self.ouvrir_autres_paiements
        )
        self.btn_autres_paiements.pack(side="right", padx=15, pady=10)

    # =====================================================================
    # LOGIQUE D'AFFICHAGE PURE (ne touche pas le stockage)
    # =====================================================================
    def mettre_a_jour_montant_conseille(self, event=None):
        """Affiche automatiquement la mensualité par défaut recommandée selon l'année d'étude"""
    ...
    # =====================================================================
    # ACTIONS DÉLÉGUÉES AU CONTRÔLEUR (lecture/écriture du stockage)
    # =====================================================================
    def charger_infos_etudiant(self, event=None):
        controleur_paiement.charger_infos_etudiant(self)

    def valider_et_generer_recu(self):
        controleur_paiement.valider_et_generer_recu(self)

    # =====================================================================
    # NAVIGATION
    # =====================================================================
    def ouvrir_autres_paiements(self):
        self.destroy()
        from vue_autres_paiements import AutresPaiementsApp
    ...
    def generer_pdf_recu(self, infos):
        """
        `infos` doit contenir : matricule, nom, annee_etude, mode_paye,
        tarif_annuel, cumul_avant, montant_verse, cumul_apres,
        reste_annuel, date_str, nom_fichier
        (voir le docstring de controleur_paiement.valider_et_generer_recu)
        """
        ...
        header_table_data = [
            [
                Paragraph("<b>REÇU OFFICIEL DE VERSEMENT</b><br/><font size=8 color='#64748B'>Paiement Scolarité Mensuelle</font>", style_grand_titre),
        ...
        col_gauche_data = [
            ...
            [Paragraph(infos['annee_etude'], style_value_normal)],
        ]
        
        col_droite_data = [
            [Paragraph("DÉTAILS DU RÈGLEMENT", style_h2)],
            [Paragraph("Libellé du Versement", style_label)],
            [Paragraph("Scolarité - Mensualité", style_value)],
            [Spacer(1, 4)],
        ...
        recap_values = [
            Paragraph(f"{infos['tarif_annuel']:,.0f} FCFA".replace(",", " "), style_cell_body),
            Paragraph(f"{infos['cumul_avant']:,.0f} FCFA".replace(",", " "), style_cell_body),
            Paragraph(f"<b>{infos['montant_verse']:,.0f} FCFA</b>", style_cell_body),
            Paragraph(f"{infos['cumul_apres']:,.0f} FCFA".replace(",", " "), style_cell_body),
            Paragraph(f"<font color='#EF4444'><b>{infos['reste_annuel']:,.0f} FCFA</b></font>".replace(",", " "), style_cell_body_bold)
        ]
